...
defmodule Astarte.Core.Generators.Triggers.SimpleEvents.SimpleEventTest do
  ...
  use Astarte.Support.Cases.Validation

  alias Astarte.Core.Generators.Triggers.SimpleEvents.SimpleEvent
  ...
  #   %SimpleEvent{}
  #   |> SimpleEvent.changeset(changes)
  # end

  # defp validation_fixture(_context), do: {:ok, validate: &validation_helper/1}

  @moduletag :simple_event
  @moduletag struct: SimpleEvent

  @doc false
  describe "triggers simple events simple event generator" do
    @describetag :success
    @describetag :ut

    property "generates valid simple event", %{validate: validate} do
      IO.inspect(validate)
