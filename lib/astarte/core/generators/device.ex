...
defmodule Astarte.Core.Generators.Device do
  ...
  @doc """
  Generates a valid Astarte Device
  """
  ...
  def device(interfaces: interfaces) do
    gen all id <- id(),
      ...
      %{
        ...
        total_received_msgs: interface_sum_data(interfaces_msgs),
        total_received_bytes: interface_sum_data(interfaces_bytes)
      }
    end
  end
  ...
  defp interface_sum_data([], acc), do: acc
  defp interface_sum_data([{_, value} | tail], acc), do: interface_sum_data(tail, acc + value)
  defp interface_sum_data(data), do: data |> Map.to_list() |> interface_sum_data(0)
  ...
  defp interfaces_data([%Interface{} | _] = interfaces) do
    ...
    one_of([interface_map, constant(nil)])
  end
