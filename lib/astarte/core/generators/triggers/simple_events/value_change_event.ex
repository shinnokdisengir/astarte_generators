...
defmodule Astarte.Core.Generators.Triggers.SimpleEvents.ValueChangeEvent do
  ...
  def value_change_event(params \\ []) do
    gen_fields =
      params gen all %{
                       interface: %Interface{interface_name: interface_name} = interface,
                       path: path,
                       endpoint: endpoint
                     } <- CommonDataEventGenerator.common_data_event(),
                     #  TODO
                     old_bson_value <-
                       ValueGenerator.value(interface: interface)
                       |> BSONValueGenerator.to_bson_value(),
                     new_bson_value <- ValueGenerator.value(interface: interface),
                     params: params do
        %{
          type: "value_change",
